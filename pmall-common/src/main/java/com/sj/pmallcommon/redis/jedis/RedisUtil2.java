...
public class RedisUtil2 {
    ...
    public Boolean put(String key, Object value, int duration, TimeUnit timeUnit) {
        ...
        String result = redisClient.invoke(jedisPool, (jedis) -> {
                    String srtResult = jedis.set(key.getBytes(), JsonUtil.objToByteArray(value));
                    if (duration <= ZERO) {
                        //默认5 分钟
    ...
    public <T> T get(String key, Class<T> clazz) {
        ...
        return JsonUtil.byteArrayToObj(result, clazz);
    }
    ...
    public boolean setNx(String key, Object value, int duration, TimeUnit timeUnit) {
        ...
        return redisClient.invoke(jedisPool, (jedis) -> {
                    long result = jedis.setnx(key.getBytes(), JsonUtil.objToByteArray(value));
                    if (result >= 1) {
                        if (duration <= ZERO) {
                            //默认5 分钟
                            jedis.pexpire(key.getBytes(), DEFAULT_CACHE_MILLIS);
    ...
    public Long append(String key, Object value) {
        validateKeyParam(key);
        return redisClient.invoke(jedisPool, (jedis) -> jedis.append(key.getBytes(),
                JsonUtil.objToByteArray(value)));
    }
    ...
    public void zAddByScore(String key, Object obj, double score) {
        ...
        redisClient.invoke(jedisPool, jedis -> {
            jedis.zadd(key.getBytes(), score, JsonUtil.objToByteArray(obj));
            return null;
        });
    }
    ...
    public <T> List<T> zRange(String key, Class<T> clazz, int start, int end) {
        ...
        List<T> result = redisClient.invoke(jedisPool, jedis -> {
            ...
            for (byte[] bytes : set) {
                T t = JsonUtil.byteArrayToObj(bytes, clazz);
                list.add(t);
            }
            return list;
        });
    ...
    public long zRank(String key, Object obj) {
        ...
        long sortIndex = redisClient.invoke(jedisPool, jedis -> {
            long result = jedis.zrank(key.getBytes(), JsonUtil.objToByteArray(obj));
            return result;
        });
        return sortIndex;
    }
    ...
    public long zRevRank(String key, Object obj) {
        validateParam(key, obj);
        long sortIndex = redisClient.invoke(jedisPool, jedis -> {
            long result = jedis.zrevrank(key.getBytes(), JsonUtil.objToByteArray(obj));
            return result;
        });
        return sortIndex;
    }
    ...
    public long zRem(String key, Object obj) {
        ...
        long lRow = redisClient.invoke(jedisPool, jedis -> {
            long result = jedis.zrem(key.getBytes(), JsonUtil.objToByteArray(obj));
            return result;
        });
        return lRow;
    }
    ...
    public Double zScore(String key, Object obj) {
        ...
        Double score = redisClient.invoke(jedisPool, jedis -> {
            Double sResult = jedis.zscore(key.getBytes(), JsonUtil.objToByteArray(obj));
            return sResult;
        });
        return score;
    }
    ...
    public Long lpush(String key, Object... value) {
        ...
        Long len = redisClient.invoke(jedisPool, jedis -> {
            return jedis.lpush(key.getBytes(), JsonUtil.objToByteArray(value));
        });
        return len;
    }
    ...
    public Long publish(String key, Object value) {
        validateParam(key, value);
        Long len = redisClient.invoke(jedisPool, jedis -> {
            return jedis.publish(key.getBytes(), JsonUtil.objToByteArray(value));
        });
        return len;
    }
    ...
    public <T> T getHash(String key, String field, Class<T> clazz) {
        ...
        if (value != null) {
            return JsonUtil.byteArrayToObj(value, clazz);
        }
        return null;
    }
        ...
        return redisClient.invoke(jedisPool, (jedis) -> {
            ...
            if (map != null) {
                for (Map.Entry<byte[], byte[]> item : map.entrySet()) {
                    ...
                    T newValue = JsonUtil.byteArrayToObj(item.getValue(), clazz);
